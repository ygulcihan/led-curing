...
void CLK_PowerDown(void)
{
    ...
    ((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->PWRCON |= ((1ul << 7) | (1ul << 6));
    __wfi();
}
...
uint32_t CLK_GetHXTFreq(void)
{
    if(((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->PWRCON & 0x01UL )
        return (12000000UL);
    else
        return 0;
}
...
uint32_t CLK_GetLXTFreq(void)
{
    if(((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->PWRCON & 0x02UL )
        return (32768UL);
    else
        return 0;
}
...
void CLK_EnableModuleClock(uint32_t u32ModuleIdx)
{
    *(volatile uint32_t *)((uint32_t)&((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->APBCLK+(((u32ModuleIdx >>31) & 0x1)*4))  |= 1<<((u32ModuleIdx >>0 ) & 0x1f);
}
